...
class Footer extends React.Component{
  ...
  subscribe = e => {
    e.preventDefault();
    if(!this.form.email.value.length || !this.form.subscribe.value.length) return;
    console.log('Sex: ' + this.form.subscribe.value +' ,Email: ' + this.form.email.value);
    this.form.style.display = 'none';
    this.text.style.display = 'block';
    localStorage.setItem('email',JSON.stringify(this.form.subscribe.value));
  }
    ...
    <footer className="footer">
      <section className="subscribe">
        <div className="subscribe__wrapper">
          ...
          <form className="subscribe__radios" action="" ref={element => this.form = element}>
            ...
            <input className="subscribe__submit" type="submit" value="ПОДПИСАТЬСЯ" onClick={this.subscribe}/>
          </form>
          <div ref={element => this.text = element} style={{display:'none',marginRight:'300px'}}>Подписка оформлена! Спасибо!</div>
        </div>
      </section>
